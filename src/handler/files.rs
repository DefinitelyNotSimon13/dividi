...
use crate::{
    state::AppState,
    templates::{FileList, HtmlTemplate},
    uploaded_file::UploadedFile,
};

pub async fn get_files(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let template = FileList::ok(state.files_as_vec().await);
...
pub async fn upload_file(
    ...
    let file_data = match file_bytes {
        Some(data) => data,
        None => {
            return handle_error("no file was uploaded", state).await;
        }
    };
    ...
    let uploaded_file = match UploadedFile::save(custom_file_name, orig_file_name, file_data).await
    {
        Ok(file) => file,
        Err(e) => {
            return handle_error(&format!("failed to save file: {}", e), state).await;
        }
    };
...
async fn handle_error(message: &str, state: Arc<AppState>) -> HtmlTemplate<FileList> {
    error!(message);

    let template = FileList::err(message.into(), state.files_as_vec().await);
    HtmlTemplate(template)
}
